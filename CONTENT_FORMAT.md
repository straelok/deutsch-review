...
Для слова и существительного используются обязательные `german` и
`translation_ru`, а также необязательные `example` и `note`. Пустые
необязательные значения не записываются:
