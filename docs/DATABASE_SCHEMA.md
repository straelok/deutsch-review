...
Статус: схема версии 2 реализована в `lib/data/database/schema.dart`, открытие и
...
запрашивает; любой активный элемент доступен для повторения. Они будут удалены
отдельной миграцией при стабилизации формата импорта.

## Границы текущего шага

Схема не включает пользователей, облачные таблицы и статистические агрегаты.
Импорт сначала проверяет пакет целиком, а затем записывает элементы в
`learning_items` одной транзакцией; реализация импорта относится к более
позднему этапу.

Поле `rating` в журнале является внутренним сигналом планировщика. Пользователь
не выбирает его вручную: значение будет вычисляться по правильности ответа,
подсказкам и числу попыток.
