...
Статус: схема версии 4 реализована в `lib/data/database/schema.dart`, открытие и
...
Миграция `3 -> 4` после автоматической резервной копии добавляет:

- `grammar_topic_progress` — синхронизируемую отметку изученности встроенной
  темы и время последнего изменения;
- `grammar_attempts` — неизменяемые ответы с ID темы, задания и занятия,
  введённым текстом, результатом и временем;
- индексы грамматических попыток по теме и занятию;
- поле `kind` со значениями `vocabulary` или `grammar` в `daily_sessions`;
- обязательные слоты 6 и 7 для грамматических занятий.

Таблица `daily_sessions` пересоздаётся с явным копированием всех строк версии
3. Существующие занятия получают тип `vocabulary`; их очередь и прогресс не
изменяются. Очередь грамматического занятия хранит снимок ID темы, выбранного
глагола и готовых заданий, поэтому начатое занятие можно точно продолжить даже
после обновления встроенного банка.

Встроенные объяснения и упражнения остаются JSON-ресурсами приложения и не
дублируются в SQLite. В базе хранятся только стабильные ID и пользовательский
прогресс.

## Локальные настройки синхронизации

В схеме версии 4 таблица `app_settings` хранит только
...
слов, словарных и грамматических попыток, отметок тем и занятий, а результат
атомарно объединяется обратно в SQLite.
