...
grant all on table public.sync_profiles to service_role;
...
begin
  ...
  loop
    ...
    if previous is null
      or coalesce(entry ->> 'updatedAt', '') >
         coalesce(previous ->> 'updatedAt', '')
      or (
        coalesce(entry ->> 'updatedAt', '') =
          coalesce(previous ->> 'updatedAt', '')
        and entry::text > previous::text
      ) then
  ...
  loop
    ...
    if previous is null
      ...
      or (
        ...
        and coalesce((entry ->> 'answeredCount')::integer, 0) >
...
begin
  ...
  insert into public.sync_profiles as profile (
    ...
    p_nickname,
    public.merge_sync_payload('{}'::jsonb, p_payload),
    1,
    now()
  )
